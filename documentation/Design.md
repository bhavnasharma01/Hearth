...
- **Home** — **warmth first, utility held inside it.** The hero keeps its full ceremonial scale (big display headline, gold rule, the one warm sentence, generous breathing room) and the **search pill lives within that warmth** (gold submit, soft placeholder "What are you looking for?…"). Below the search: a proper **"＋ Add your practice" outline button** (practitioner recruitment must be *visible*, not a text link) beside a quiet "Browse all →". Then the **"Browse by need" category rail** (paths in, right after the welcome), then the directory peek. Lesson learned (Build 35): the heartiness lives in **scale, breath, the human sentence, and gold** — trimming those for utility density makes the page feel transactional.
