...
- **"The website of your home page URL is not registered to you":** Google checks domain ownership via **Search Console** under the same Google account. Fix: search.google.com/search-console → Add property → **Domain** type → `myhearthapp.ca` → add the `google-site-verification` **TXT record at Porkbun** (root/blank host) → Verify in Search Console → retry the Cloud-console submission.
- **"Your app needs to be verified" / Verify button disabled:** Google's brand verification requires an **Application home page** (`https://www.myhearthapp.ca`) and a **Privacy policy** URL — use **`https://www.myhearthapp.ca/privacy`** (built in Build 76, footer-linked). Fill both, save, submit for verification; the review takes days and sign-in keeps working meanwhile (just with the raw domain shown until approval).
- **The Google screen shows `lygcgrbdvmsqzmbhdlmv.supabase.co` instead of "Hearth":** Google headlines the OAuth redirect's domain (the Supabase project) when consent-screen branding is incomplete. Free fix (usually makes the headline "to continue to Hearth"): on the OAuth consent screen confirm **App name = Hearth**, a **support email**, **Authorized domains includes `myhearthapp.ca`**, and **Publishing status = In production**. Even then Google may keep the supabase.co address in small print — fully removing it requires a **Supabase custom domain** (Pro $25/mo + $10/mo add-on → `auth.myhearthapp.ca`), a deliberate cost decision deferred unless it confuses the community.
