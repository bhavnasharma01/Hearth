...
- **Public read, live-only:** the anon role can `SELECT` only rows with `status = 'live'` (practitioners, events) and `categories` where `active = true`. `practitioner_categories` is visible only for live practitioners. This is enforced by policy in `supabase/migrations/0001_initial_schema.sql`.
  - **RLS is viewer-dependent — public reads also filter status in the query.** Authenticated users (admins/stewards) get the `..._admin_all` policy (`for all to authenticated using (true)`), which grants **every** status. Because the public server client uses the anon key **plus the visitor's session cookie**, a signed-in admin browsing the public site is `authenticated` and would see hidden/pending rows through that policy. So the public data reads (`getPractitioners`, `getEvents`, `getPractitionerBySlug`, …) filter `status = 'live'` **explicitly in the query** — the app never relies on RLS alone to hide non-live content from a privileged viewer. RLS remains the backstop for true anon (a leaked/misused anon key still can't read non-live rows). *(A missing explicit filter on `getPractitioners`/`getEvents` was the "Hide doesn't remove it from the live site" bug — Build 43.)*
