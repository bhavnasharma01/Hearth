...
- **Public surface — no auth to consume.** Browsing, searching, contacting, and reporting require no account. This is a deliberate product principle, not an oversight.
- **Member sign-in exists (accounts Phase A, Build 46) — login gates *contributing*, never *consuming*.** Members can sign in with **Google** (Supabase OAuth) at `/signin` (subtle header control, `AccountControl`; OAuth code exchanged at `/auth/callback`). A signed-in member's data access equals an anonymous visitor's (live-only public reads) **plus** read/update of their own `users` row — nothing more. A profile row is auto-created on first sign-in (DB trigger, migration `0008`); a signed-in practice submission binds `owner_user_id`. **Email/password sign-in shipped in Build 54** (myhearthapp.ca verified in Resend): sign-up requires **email confirmation** (Supabase "Confirm email" ON — no made-up addresses), password reset flows through `/auth/callback` → `/reset-password` (session-gated; an expired link bounces to `/signin` with a gentle error), and all auth emails send via Supabase custom SMTP through Resend from the verified domain.
- **Owning a listing (Phase B, Build 47).** `/my-listing` (session-gated) lets the owner edit without any link; adding a practice now asks for sign-in first (the just-in-time moment — the listing binds to the account). **Claiming** links a pre-account listing to an account via two verified paths only: the session email matching the listing's contact email (re-derived server-side, never a client-supplied id), or holding the listing's manage link while signed in (the token already proves edit rights, so linking is a strict upgrade). Only **unowned** listings can be claimed. Owners can also **delete** their listing (token-authorized, same trust model as editing).
...
- **Public read, live-only:** the anon role can `SELECT` only rows with `status = 'live'` (practitioners, events) and `categories` where `active = true`. `practitioner_categories` is visible only for live practitioners. This is enforced by policy in `supabase/migrations/0001_initial_schema.sql`.
  - **RLS is viewer-dependent — public reads also filter status in the query.** The public server client uses the anon key **plus the visitor's session cookie**, so a signed-in viewer is `authenticated`, and RLS policies for that role apply. Under the v1 `..._admin_all` policies this meant a signed-in admin browsing the public site saw hidden/pending rows (the "Hide doesn't remove it from the live site" bug — Build 43). The fix stands as policy: public data reads (`getPractitioners`, `getEvents`, `getPractitionerBySlug`, …) filter `status = 'live'` **explicitly in the query**, never relying on RLS alone. Migration `0008` also closed the path at the DB layer by dropping `..._admin_all`; the explicit filters remain as defence-in-depth.
...
- **`authenticated` is de-privileged (migration `0008`).** The v1 `..._admin_all` policies (full CRUD for any signed-in user — safe only while admins were the sole users who *could* sign in) are **dropped**. A signed-in member now has the same RLS access as anon, plus `select`/`update` on **their own** `users` row (`auth.uid() = id`). Admin reads/writes never used those policies (service role throughout), so nothing admin-side changed.
...
- **Photo uploads go through a service-role action, not anon.** The public **`avatars`** Storage bucket is public-**read** (images display on public profiles), but has **no anon write policy** — uploads happen only via the `uploadAvatar` service-role action, which validates content-type (JPG/PNG/WebP) + size (≤2 MB) and generates a random path. So a leaked anon key can't write to storage. *(Open-endpoint flood is still on the rate-limit watchlist — see `Bugs.md`.)* *(Build 24.)*
- **Testimonials (`0009`, Build 60): public read = approved-on-live only.** No anon/authenticated write policies — submit/approve/hide/delete are service-role actions that re-derive identity from the session: submit requires sign-in, is content-checked (links/spam blocked outright), rejects the owner recommending themselves, and is unique per (practitioner, author); approve/hide verify the session user **owns** the target practitioner; delete verifies authorship. Nothing becomes public without the practitioner's explicit approval.
- **Owner edit is capability-based, not a login (`manage_token`).** A practitioner edits their own listing via a secret `/manage/<manage_token>` URL — an unguessable UUID that grants edit rights to **that one listing only** (never admin). It's **column-revoked from the `anon`/`authenticated` roles** (`revoke select (manage_token)`) so it can't leak through a public `select *`, and is only ever read via the service-role. Edits re-run the content-check (a flagged live listing is held for review), so the link can't push spam public. Treat the link like a password; it can be rotated by reissuing the token. *(Build 23.)* **Admins can also view/copy a listing's manage link and edit via it** from `/admin/listings` (`listPractitionersAdmin` selects `manage_token` with the service role) — this is safe **only** because that read is rendered exclusively inside the auth-gated `/admin` area; the token is never added to a public/anon read. *(Build 41.)*
...
- **No tracking, no ads, no third-party analytics** beyond what's strictly needed; nothing that profiles visitors. *(One deliberate exception since Build 60: practitioner profiles embed a lazy-loaded Google **neighbourhood map** — third-party content Google may set cookies on — because "how do I get there" is core user value. The zoom stays neighbourhood-level no matter how precise the shared location is, so the map never pinpoints a door.)*
- **"Near me" location** is used only to build the page URL (`?lat=&lng=`) so the server can sort by distance — it is **never stored server-side, logged, or sent to any third party**. Geolocation requires the visitor's explicit per-use permission (and HTTPS). Practitioner coordinates come from whatever location the practitioner chose to publish in `area` (**as specific as a street address or as general as a city**, per the form's "whatever you're comfortable sharing" framing, Build 86); the profile map stays neighbourhood-zoom regardless.
