...
**Practitioners tab** — search (name/practice/description/keywords) + category chips + In person/Online/Both filter. Cards: **practice and practitioner name together** ("Practice · Person" when they differ, Build 94 — people recognize the person from WhatsApp), category tags, description, area + mode, **Community-member badge**, photo, and contact buttons (*Message on WhatsApp* via `wa.me`, Email, Website, Instagram); a floating back-to-top button appears on long scrolls. Each card opens a **profile page** (`/p/slug`) with full bio, categories, contacts, a Where & how card (location + map + directions), services, **"Kind words"** (approved recommendations + the ♡ Recommend gateway), and **upcoming events this person hosts**. Prominent **➕ Add your practice**; subtle **Report a listing**.
...
**The list has grown in practice** (21 by July 14: Womb Healing, Meditation, Breathwork, Sound Healing, Voice Activation, Men's/Women's Work, Astrology, Conscious Products, …). Since Build 93 categories display **alphabetically** (app + admin) and admins can rename (slug stays stable), deactivate (in use), or delete (unused) with visible feedback.
...
- **v1 — two layers, one home** *(current build):* Directory (add/browse/search/filter/profile/contact/member badge) · Events (native add/upcoming feed/filters/month view/seeded import) · moderation (auto-check + reporting) · admin panel · category management.
  - **Pilot note (Build 92) — adoption strategy: supply first.** Decision (July 11): recruit practitioners before marketing to seekers. A seeker gives us one first impression, and a thin directory wastes it (per §1, the problem is real but not acute, so nobody comes back to an empty hall); a robust directory makes every later seeker touchpoint convert. Practically: the printable QR cards (`/qr-card.html`) lead with the practitioner-recruitment pages (QR straight to Add your practice), and the pitch capitalizes on the ask being one-time and small: add it once, about two minutes, yours to tend anytime. Seeker-facing marketing follows once the directory feels alive.
...
7. **Category taxonomy rethink — parked (July 14).** The flat list is growing fast (21 and counting) as real practitioner variety shows up, and Bhavna wants to explore **categories + subcategories** (e.g. modality families) rather than one long rail. Not urgent: alphabetical ordering + admin tools (Build 93) keep the flat list workable for the pilot. When picked up, consider: how subcategories surface on the rail without clutter, what happens to existing practitioner links, and whether search (weight B on categories) should also index the parent.
8. **Retreats — parked (July 6 call).** A real community need (people host retreats in Ontario and abroad) with no good home yet. Decided **against** a practitioner-directory category: a directory row can't carry dates, and seekers don't look for retreats in a directory (Anat's end-user argument). Likely home is the events layer when it returns; a WhatsApp retreats-only channel is the stopgap. Revisit when `EVENTS_ENABLED` flips back on.
