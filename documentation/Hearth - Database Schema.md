...
4. **Contact requirement — at least one.** Every practitioner must provide at least one of WhatsApp / email / website / **Instagram**. Enforced app-side and with a DB constraint: `CHECK (whatsapp IS NOT NULL OR email IS NOT NULL OR website IS NOT NULL OR instagram IS NOT NULL)`. *(Instagram added in migration `0003_instagram_contact.sql`, Build 17.)*
...
8. **`practitioner_services` (Build 25, migration `0007`).** The "what I offer" menu per practitioner — `title` (req), optional `description`/`price_note`, `sort_order`. Public-read for **live** practitioners (mirrors `practitioner_categories`); writes via the service-role manage action. Managed from `/manage/<token>`, rendered as "What I offer" on the profile.
