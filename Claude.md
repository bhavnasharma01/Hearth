...
4. **Brand decision — logo CHOSEN, palette open.** ✓ **The mark is L2, the heart-flame** (a heart holding a flame cutout; decided 2026-07-10 after finding hearthapp.ca's similar flame — the name stays, the old flame mark must NOT ship). Palette: 6 finalists on `/palette-explorations.html` (F6 Clementine & juniper is Bhavna's lean; she's consulting **Greg**, not Anat/Curtis, on this one); site wears the Rice Paper trial meanwhile. The palette pick triggers one combined build: refined heart-flame SVG in the header (`logo.tsx`, never simplified to a plain heart), **favicon** (`src/app/icon.svg`), palette swap, **branded Supabase auth-email templates**, and Design.md §3 refresh.
