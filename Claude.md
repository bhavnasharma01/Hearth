...
- **Roadmap:** 1c = **services menu** (next). Then Phase 2 **testimonials** (solicited + positive only — NOT open reviews; see `Product.md §6`), reusing the same capability-link pattern for "request a testimonial." The `owner_user_id` column is the v2 upgrade path (real accounts).
