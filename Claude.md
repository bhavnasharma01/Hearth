...
- Editing is via a **secret capability URL**: `/manage/<manage_token>`. The token (uuid on `practitioners`, migration `0005`) grants edit to *that one listing only* — never admin. Surfaced on the add-practitioner success screen ("your private edit link — bookmark it"). **Since accounts Phase B (Build 47) the token is the bridge, not the only path:** owners edit from `/my-listing` (session-gated; internally reuses `ManageForm` + the listing's own token), claim an unowned listing by session-email match or by opening the manage link signed in (`src/lib/actions/account.ts`, all re-verified server-side), and can **delete** their listing (`DeleteListing`, token-authorized, on both edit surfaces). `/add-practitioner` asks for sign-in first; the submit binds `owner_user_id`.
