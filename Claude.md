...
- **Category seed = 11 rows** (see schema §`categories`). Categories are a table, not a hardcoded list — admins add more. The directory's **`CategoryRail`** (`src/components/category-rail.tsx`) maps each seeded slug to an icon + short label; an **admin-added category falls back** to a neutral `✻` + the first words of its name, so nothing breaks — but add a proper entry to `CATEGORY_META` when a new category matters.
